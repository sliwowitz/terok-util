...
def _outer_in_container(slot_name: str) -> str:
    """The slot's root-side script, as the container sees it on the results mount."""
    return f"{RESULTS_MOUNT}/outer-{slot_name}.sh"
...
def _run_argv(
    ...
    argv += [
        "--name",
        _container_name(config, slot_name),
